...
---Liệt kê các hóa đơn của khách hàng (mã user, tên user, mã hóa đơn)
SELECT users.user_id, users.user_name, orders.order_id
FROM users
JOIN orders ON users.user_id = orders.user_id;
---Liệt kê số lượng hóa đơn của khách hàng (mã user, tên user, số đơn hàng)
...
SELECT orders.order_id, users.user_id, users.user_name, 
       GROUP_CONCAT(products.product_name ORDER BY products.product_name SEPARATOR ', ') AS product_names
FROM users
JOIN orders ON users.user_id = orders.user_id
JOIN order_details ON orders.order_id = order_details.order_id
...
---Liệt kê 7 người dùng có số lượng đơn hàng nhiều nhất
SELECT users.user_id, users.user_name, COUNT(orders.order_id) AS total_orders
FROM users
JOIN orders ON users.user_id = orders.user_id
GROUP BY users.user_id, users.user_name
...
LIMIT 7;
---Liệt kê 7 người dùng mua sản phẩm có tên chứa "Samsung" hoặc "Apple"
...
LIMIT 7;
---Liệt kê danh sách mua hàng của user bao gồm giá tiền của mỗi đơn hàng
SELECT users.user_id, users.user_name, orders.order_id, 
...
---Liệt kê danh sách mua hàng của user (chỉ lấy đơn hàng có giá trị lớn nhất)
